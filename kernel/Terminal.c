...
VOID DrawChar(CHAR Char, UINT32 PositionX, UINT32 PositionY, UINT32 Color) {
  for (UINT32 Y = 0; Y < 8; Y++) {
    ...
    UINT8 FontByte = sFontBasic[(UINT8)Char][Y];

    for (UINT32 X = 0; X < 8; X++) {

      UINT8 PixelSet = FontByte & (1 << (7 - X));

      if (PixelSet) {
        DrawPixel(PositionX + X, PositionY + Y, Color);
      }
    }
  }
}
VOID DrawString(STRING String, UINT32 PositionX, UINT32 PositionY, UINT32 Color) {
  for (UINT32 I = 0; String[I] != 0; I++) {
    DrawChar(String[I], PositionX + (I * 8), PositionY, Color);
  }
}